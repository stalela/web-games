This behavior is caused by a **visual-to-logic mapping mismatch**. 

In International Draughts (10x10), pieces only move on the dark squares. In your screenshot, your white pieces are on the **Dark** squares starting from the top-left (0,0), but your code logic calculates playable squares using `(row + col) % 2 === 1`. This makes the logic think the light squares are the playable ones.

When you click a white piece, the engine shows "legal moves," but because of the mismatch, it places the grey dots on the light squares. When you click a dot, the engine rejects it as an "off-board" move because it's not a dark square according to the logic.

Here is the corrected code. I have updated the **mapping functions** and the **board creation** to ensure everything aligns perfectly with the standard 10x10 rules.
...
export class CheckersGame extends Phaser.Scene {
  ...
  handleMoveAttempt(to) {
      if (this.makeMove(this.from, to)) {
          ...
          this.turnText.setText("Black's turn");
          this.from = -1;
          this.selectedPiece = null;
  ...
  makeAIMove() {
    ...
    this.currentPlayer = 'W';
    this.turnText.setText("White's turn");
...
### Key fixes applied:
1.  **Playable square logic:** Changed `isPlayableTile` to `(row + col) % 2 === 0`. This ensures that square (0,0) is dark/playable, matching your assets and the International Draughts pattern.
2.  **Coordinate Mapping rewritten:**
    *   `viewPosToEngine`: Now correctly maps visual top row (0-9) to the engine's white starting area (bottom indices).
    *   `engineToViewPos`: Added a parity check `(row % 2)` to the column calculation to handle the "staggered" nature of checkerboard rows.
3.  **Tile interaction:** Tiles are now correctly identified and only playable tiles accept "pointerdown" events to complete a move.
4.  **Board Rendering:** Updated `createBoard` so every square is drawn with the correct alternating texture, eliminating the "one color" appearance.